 ...
 * @param callback
 */
function getRequest(path, options, config, callback) {
  ...
  sendRequest(path, options, callback,config);
}
...
function sendRequest(path, options, callback,config) {
  var requestOptions = url.parse(path, true);
  requestOptions.method = options.method;
  requestOptions.cer = options.cer;
  if (options.headers) {
  ...
  requestOptions.requestCert=config.requestCert;  //请求客户端证书
  requestOptions.rejectUnauthorized=config.rejectUnauthorized; //默认为true，如果没有请求到客户端来自信任CA颁发的证书，拒绝客户端的连接
